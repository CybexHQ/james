...
impl Default for NetworkRuntimeOptions {
    fn default() -> Self {
        Self {
            config_path: PathBuf::from("/etc/cybex-pulse/config.toml"),
            network_plan_path: PathBuf::from("/var/lib/cybex-pulse/control/netplan-approved.json"),
            fallback_network_plan_path: PathBuf::from(
                "/var/lib/cybex-pulse/control/netplan-dhcp-fallback.json",
            ),
            fallback_marker_path: PathBuf::from(
                "/var/lib/cybex-pulse/control/network-fallback-active",
            ),
...
pub fn reconcile_network_runtime(options: NetworkRuntimeOptions) -> Result<NetworkRuntimeOutcome> {
    ...
    let parsed = crate::config::AppConfig::from_toml_str(&config, &options.config_path)
        .context("validate Pulse configuration before network reconciliation")?;
    ...
    crate::config::AppConfig::from_toml_str(&config, &options.config_path)
        .context("validate reconciled Pulse configuration")?
        .validate_appliance_config()
        .context("validate installed appliance configuration")?;
...
fn fallback_marker_matches_approved(
    ...
    if marker.schema != "cybex.pulse.network-fallback.v1"
    ...
    {
        bail!("Pulse network fallback marker is invalid")
    }
...
fn approved_runtime_network(plan: &ApprovedNetplan) -> Result<ApprovedRuntimeNetwork> {
    if plan.network.version != 2 || plan.network.renderer != "networkd" {
        bail!("approved Pulse network plan has an unsupported network shape")
    }
    if plan.network.ethernets.len() != 1 {
        bail!("approved Pulse network plan must contain exactly one wired interface")
    }
    let interface = plan
        .network
        .ethernets
        .get("cybex-pulse")
        .ok_or_else(|| anyhow!("approved Pulse network plan omitted its managed interface"))?;
    if interface.dhcp6 {
        bail!("approved Pulse network plan must keep DHCPv6 disabled")
    }
...
fn replace_server_public_base_url(config: &str, desired: &str) -> Result<String> {
    ...
    for raw_line in config.split_inclusive('\n') {
        ...
        if in_server && trimmed.starts_with("public_base_url") {
            ...
            if key != "public_base_url" {
                bail!("Pulse server public URL declaration is malformed")
            }
    ...
    if replaced != 1 {
        bail!("Pulse configuration must contain one server public URL")
    }
    Ok(output)
}
...
fn atomic_replace_preserving_metadata(path: &Path, body: &[u8]) -> Result<()> {
    let metadata = fs::symlink_metadata(path).context("inspect Pulse configuration metadata")?;
    if !metadata.file_type().is_file() {
        bail!("Pulse configuration must be a regular file")
    }
    let parent = path
        .parent()
        .ok_or_else(|| anyhow!("Pulse configuration has no parent directory"))?;
    ...
    let result = (|| -> Result<()> {
        let mut file = OpenOptions::new()
            ...
            .open(&temporary)
            .context("create temporary Pulse configuration")?;
        let owner_status = unsafe {
            use std::os::fd::AsRawFd;
            libc::fchown(file.as_raw_fd(), metadata.uid(), metadata.gid())
        };
        if owner_status != 0 {
            return Err(std::io::Error::last_os_error()).context("preserve Pulse config owner");
        }
        file.set_permissions(fs::Permissions::from_mode(metadata.mode() & 0o7777))?;
        file.write_all(body)?;
        file.sync_all()?;
        fs::rename(&temporary, path).context("activate reconciled Pulse configuration")?;
...
mod tests {
    ...
    fn stale_fallback_marker_cannot_override_newly_approved_plan() {
        ...
        let marker = NetworkFallbackMarker {
            schema: "cybex.pulse.network-fallback.v1".to_string(),
