...
class Workstation:
    def __init__(self, state):
        ...
        self.hardware = SCOPE['hardware_identity'](self.scope, 'workstation')
        self.bridge = self.scope['bridge']
        self.state = state
        self.tap = None
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.process = subprocess.Popen(['qemu-system-x86_64', '-enable-kvm', '-machine', 'q35,smm=on', '-global', 'driver=cfi.pflash01,property=secure,value=on',
                ...
                '-netdev', 'tap,id=net0,ifname=' + self.tap + ',script=no,downscript=no',
                '-device', 'virtio-net-pci,netdev=net0,mac=' + self.hardware['mac'] + ',bootindex=1',
    ...
    def __exit__(self, *args):
        ...
        if self.tap:
            SCOPE['tap'](self.state, self.scope['manage_origin'], self.bridge, 'workstation', False)
            self.tap = None
    ...
    with Workstation(state) as workstation:
        ...
        def pending():
            ...
            matches = [v for v in rows['enrollments'] if v.get('device_kind') == 'nixos-installer'
                       and str(v.get('facts_json', {}).get('primary_mac', '')).lower() == workstation.hardware['mac']
            ...
            booted = any(str(v.get('mac', '')).lower() == workstation.hardware['mac']
