    ...
    def dispatch(self, value):
        ...
        if operation == 'personalize' and set(value) == {'operation', 'path', 'secret'}:
            ...
            body = self.owner.client(receipt).request_bytes(value['path'], token=token,
                headers={'X-Cybex-James-Provisioning-Secret': value['secret']})
            if len(body) != 8192:
...
def main():
    ...
    parser.add_argument('operation', choices=('api', 'personalize', 'allow-device'))
    ...
    if args.operation == 'api':
        ...
        body = base64.b64decode(request(args.state_dir, 'personalize', path=args.path, secret=secret), validate=True)
