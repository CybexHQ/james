...
FIRST_BOOT_SERVICE = SERVICE.with_name("cybex-pulse-first-boot.service")
PHYSICAL_CONSOLE_SERVICE = SERVICE.with_name("getty@tty1.service")
NETWORK_RUNTIME_SERVICE = SERVICE.with_name("cybex-pulse-network-runtime.service")
NETWORK_RUNTIME_TIMER = SERVICE.with_name("cybex-pulse-network-runtime.timer")
NETWORK_RUNTIME = FIRST_BOOT.with_name("cybex-pulse-network-runtime")
PHYSICAL_CONSOLE = FIRST_BOOT.with_name("cybex-pulse-console")
STATE_LAYOUT = FIRST_BOOT.with_name("cybex-pulse-state-layout")
IPXE_AUTOEXEC = (
    REPOSITORY
    / "ubuntu-appliance"
    / "rootfs"
...
PXE_NGINX_SITE = (
    REPOSITORY
    / "ubuntu-appliance/rootfs/etc/nginx/sites-available/cybex-pulse"
)
APPLIANCE_UPDATE = FIRST_BOOT.with_name("cybex-pulse-appliance-update")
GENERATION_COMMIT = FIRST_BOOT.with_name("cybex-pulse-generation-commit")
POSTINST = REPOSITORY / "ubuntu-appliance" / "package" / "cybex-pulse-appliance.postinst"
...
NETPLAN_ACTIVATE = NETPLAN_APPLY.with_name("cybex-pulse-netplan-activate")
...
class ApplianceFirstBootContractTests(unittest.TestCase):
    ...
    def test_local_cybex_packages_are_byte_reproducible(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            directory = Path(temporary)
            pulse = directory / "cybex-pulse"
            bootstrap = directory / "cybex-pulse-bootstrap"
            pulse.write_bytes(b"deterministic Pulse fixture\n")
            ...
            appliance = next(
                ...
                if package.name.startswith("cybex-pulse-appliance_")
            )
            pulse_package = next(
                package
                for package in first_packages
                if package.name.startswith("cybex-pulse_")
            )
            pulse_data_root = directory / "pulse-data"
            subprocess.run(
                ["dpkg-deb", "--extract", str(pulse_package), str(pulse_data_root)],
                check=True,
            )
            packaged_source_dir = (
                pulse_data_root / "usr/share/cybex-pulse/manage-source"
            )
            ...
            with tarfile.open(outer_tar, mode="w", format=tarfile.USTAR_FORMAT) as archive:
                member = tarfile.TarInfo(pulse_package.name)
                member.size = pulse_package.stat().st_size
                member.mode = 0o644
                member.uid = 0
                member.gid = 0
                member.mtime = 0
                with pulse_package.open("rb") as package_file:
            ...
            release_tool = runpy.run_path(str(REPOSITORY / "tools/pulse-release.py"))
            ...
            packaged_first_boot = (
                data_root
                / "usr/lib/cybex-pulse/cybex-pulse-first-boot"
            ...
            packaged_physical_console = (
                data_root / "usr/lib/cybex-pulse/cybex-pulse-console"
            )
            packaged_physical_console_service = (
                data_root / "etc/systemd/system/getty@tty1.service"
            )
            packaged_ipxe_autoexec = (
                data_root / "usr/share/cybex-pulse/autoexec.ipxe"
            )
            packaged_pxe_menu_background = (
                data_root
                / "usr/share/cybex-pulse/assets/pxe-menu.png"
            )
            packaged_pxe_nginx_site = (
                data_root / "etc/nginx/sites-available/cybex-pulse"
            )
            ...
            self.assertIn(
                "alias /usr/share/cybex-pulse/assets/pxe-menu.png;",
                pxe_nginx,
            )
            self.assertIn(
                "10-netplan-cybex-pulse.network",
    ...
    def test_thin_iso_and_package_snapshot_are_built_separately(self) -> None:
        ...
        self.assertIn('Boot Cybex Pulse Setup', template)
        self.assertIn('set theme=/boot/grub/themes/cybex-pulse/theme.txt', template)
        ...
        self.assertIn("usr/share/cybex-pulse/manage-source", package_builder)
        ...
        self.assertIn("cybex.pulse.manage-source.v1", source_builder)
        ...
        self.assertIn(
            'CYBEX_PULSE_BUILD_MANAGE_ORIGIN="$CYBEX_PULSE_BUILD_MANAGE_ORIGIN"',
            workflow,
        )
        self.assertIn(
        ...
        self.assertIn(
            '"$RUNNER_TEMP/cybex-pulse-appliance-packages-metadata.json"',
            workflow,
        )
        self.assertIn('--installer-iso-template-metadata', workflow)
        self.assertNotIn(
            'CYBEX_PULSE_BUILD_MANAGE_ORIGIN="$("$RUNNER_TEMP/cybex-pulse-bootstrap"',
            workflow,
        )
        self.assertNotIn(
            "ubuntu-appliance/rootfs/usr/lib/cybex-pulse/*", workflow
        )
        self.assertIn("python3 -m py_compile", workflow)
        self.assertIn("squashfs-tools", workflow)
        self.assertIn(
            "cargo:rerun-if-env-changed=CYBEX_PULSE_BUILD_MANAGE_ORIGIN",
    ...
    def test_template_rejects_a_production_bootstrap_for_an_explicit_dev_origin(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            output = root / "output"
            output.mkdir()
            bootstrap = root / "cybex-pulse-bootstrap"
    ...
    def test_previous_package_snapshot_is_only_an_explicit_authenticated_cache_seed(self) -> None:
        ...
        self.assertNotIn("CYBEX_PULSE_PREVIOUS_PACKAGE_SNAPSHOT", snapshot_builder)
        self.assertNotIn("CYBEX_PULSE_PREVIOUS_PACKAGE_SNAPSHOT", offline_builder)
        ...
        self.assertIn('package.name.lower().startswith("cybex-pulse")', extractor)
        self.assertLess(
    ...
    def test_service_can_read_config_only_after_first_boot_succeeds(self) -> None:
        ...
        self.assertIn(
            "Requires=cybex-pulse-first-boot.service "
            "cybex-pulse-network-runtime.service\n",
            service,
        )
        self.assertIn("cybex-pulse-network-runtime.service", service)
    ...
    def test_physical_console_is_status_only_and_preserves_recovery_paths(self) -> None:
        ...
        self.assertIn("ExecStart=/usr/lib/cybex-pulse/cybex-pulse-console", service)
        ...
        self.assertIn("CYBEX PULSE", console)
        ...
        self.assertIn("root:cybex-pulse:640:1", console)
...
printf '%s:%s\\n' "$appliance_state" "$non_ready_checks"
unit_failed() {{ [[ "$1" = cybex-pulse.service ]]; }}
update_appliance_state
printf '%s:%s\\n' "$appliance_state" "$non_ready_checks"
""",
            ],
    ...
    def test_service_nix_client_state_is_private_without_weakening_control(self) -> None:
        ...
        expected_environment = {
            "HOME": "/var/cache/cybex-pulse/agent/home",
            "XDG_CACHE_HOME": "/var/cache/cybex-pulse/agent/cache",
            "XDG_CONFIG_HOME": "/var/cache/cybex-pulse/agent/config",
            "XDG_STATE_HOME": "/var/cache/cybex-pulse/agent/state",
            "TMPDIR": "/var/cache/cybex-pulse/agent/tmp",
        ...
        self.assertIn(
            "ReadWritePaths=/var/lib/cybex-pulse /var/cache/cybex-pulse\n",
            service,
        )
    ...
    def test_root_network_helper_shares_handshake_files_with_pulse(self) -> None:
        ...
        self.assertIn('chown root:cybex-pulse "$temporary"\n', change_script)
        ...
        guard = FIRST_BOOT.with_name("cybex-pulse-network-guard").read_text(
            encoding="utf-8"
        )
        self.assertIn('cybex.pulse.network-fallback.v1', guard)
        ...
        self.assertIn("10-netplan-cybex-pulse.network", activation)
    ...
    def test_netplan_activation_preserves_readability_and_rejects_dracut(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            active = root / "90-cybex-pulse.yaml"
            runtime = root / "network"
            fake_bin = root / "bin"
            calls = root / "netplan-calls"
            runtime.mkdir()
            fake_bin.mkdir()
            expected = runtime / "10-netplan-cybex-pulse.network"
    ...
    def test_bridge_state_boundary_and_package_staging_are_fail_closed(self) -> None:
        postinst = POSTINST.read_text(encoding="utf-8")
        harden_parent = postinst.index('chown root:cybex-pulse "$pulse_root"')
        inspect_child = postinst.index('for protected in "$pulse_root/control" "$pulse_root/status"')
        ...
        self.assertIn(
            '"/var/lib/cybex-pulse/status/reliability-state.json"', manage
        )
        self.assertNotIn(
            '"/var/lib/cybex-pulse/reliability-state.json"', manage
        )
        first_boot = FIRST_BOOT.read_text(encoding="utf-8")
        self.assertNotIn('chown -R cybex-pulse:cybex-pulse /var/lib/cybex-pulse/state', first_boot)
        ...
        seal = updater.index('candidate_control="$candidate_path/var/lib/cybex-pulse/control"')
        ...
        arm_candidate = updater.index('grub-reboot "cybex-pulse-generation-$candidate"')
    ...
    def test_orphan_cleanup_is_bounded_and_preserves_active_bundle(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            request.write_text(
                json.dumps(
                    {
                        "schema": "cybex.pulse.appliance-update-request.v1",
    ...
    def test_tftp_assets_and_runtime_address_are_fail_closed(self) -> None:
        ...
        self.assertIn('install -d -m 0755 -o root -g root /var/cache/cybex-pulse/tftp', first_boot)
        ...
        self.assertIn("! -L /var/cache/cybex-pulse/tftp", first_boot)
        self.assertIn('TFTP_USERNAME="tftp"', first_boot)
        self.assertIn('TFTP_DIRECTORY="/var/cache/cybex-pulse/tftp"', first_boot)
        self.assertIn('TFTP_OPTIONS="--secure"', first_boot)
        self.assertNotIn('TFTP_USERNAME="cybex-pulse"', first_boot)
        self.assertLess(
            first_boot.index("cybex-pulse-network-guard"),
            first_boot.index("cybex-pulse-network-runtime"),
        )
        self.assertLess(
            first_boot.index("cybex-pulse-network-runtime"),
        ...
        self.assertIn("Before=cybex-pulse.service", service)
        ...
        postinst = POSTINST.read_text(encoding="utf-8")
        self.assertIn("cybex-pulse-network-runtime.timer", postinst)
        self.assertIn("install -d -m 0755 -o root -g root /var/cache/cybex-pulse/tftp", postinst)
        self.assertIn("! -L /var/cache/cybex-pulse/tftp", postinst)
