...
class LegacyBridgeGateTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.snapshot_bundle = self.write(
            "cybex-pulse-appliance-packages-0.2.1-dev.13-x86_64-linux.tar.zst",
            ...
            canonical(
                {
                    ...
                    "required_package_versions": {"cybex-pulse": "2"},
        ...
        self.status = b"Package: cybex-pulse\nStatus: install ok installed\nVersion: 1\n"
        ...
        self.identity = {
            "schema": "cybex.pulse.published-appliance-predecessor.v1",
        ...
        self.policy = {
            "schema": "cybex.pulse.legacy-update-bridge-policy.v1",
            ...
            "allowed_upgrades": [
                {"package": "cybex-pulse", "from": "1", "to": "2"}
            ],
            "allowed_additions": [],
        }
    ...
    def package_set_sha256(self) -> str:
        package = self.packages / "cybex-pulse_2_all.deb"
    ...
    def evidence(
        ...
        *,
        ...
        value = {
            "schema": "cybex.pulse.legacy-update-bridge-evidence.v1",
            "ok": True,
    ...
    def test_packaged_updater_identity_derives_legacy_contract(self) -> None:
        ...
        updater = (
            package_root
            / "usr/lib/cybex-pulse/cybex-pulse-appliance-update"
        )
        packaged_release = package_root / "usr/share/cybex-pulse/appliance-release.json"
        ...
        (package_root / "usr/lib/cybex-pulse").chmod(0o755)
        ...
        repository.mkdir()
        package = repository / "cybex-pulse-appliance_1-1_amd64.deb"
        subprocess.run(
    ...
    def test_candidate_byte_drift_is_rejected(self) -> None:
        ...
        with (self.packages / "cybex-pulse_2_all.deb").open("ab") as package:
    ...
    def test_capture_source_hard_codes_the_real_legacy_wildcard_paths(self) -> None:
        ...
        self.assertIn(
            'INSTALLED_STATE_PATH = Path("/var/lib/cybex-pulse/control/appliance-release.json")',
            source,
        )
        self.assertIn(
    ...
    def test_publish_recheck_rejects_concurrent_predecessor_drift(self) -> None:
        manifest = {
            ...
            "artifact": {
                "url": (
                    ...
                    "v0.2.1-dev.11/cybex-pulse-x86_64-linux"
                ),
                "sha256": "7" * 64,
            },
            "appliance_release_v1": {
                ...
                "cybex_repository_snapshot": {
                    "url": (
                        "https://github.example/CybexHQ/forge/releases/download/"
                        "v0.2.1-dev.11/cybex-pulse-appliance-packages-"
        ...
        compatibility = {
            "schema": "cybex.pulse.release-compatibility.v1",
            "pulse_release_version": "0.2.1-dev.11",
            "release_manifest": {
                "url": (
                    "https://github.example/CybexHQ/forge/releases/download/"
                    "v0.2.1-dev.11/cybex-pulse-release.json"
                ),
    ...
    def test_workflow_blocks_signing_on_governed_exact_candidate_gate(self) -> None:
        ...
        self.assertIn("CYBEX_PULSE_LEGACY_BRIDGE_POLICY_SHA256", workflow)
        self.assertIn("CYBEX_PULSE_LEGACY_BRIDGE_EVIDENCE_SHA256", workflow)
        ...
        self.assertNotIn("CYBEX_PULSE_PREDECESSOR_UPDATE_CONTRACT", workflow)
        ...
        self.assertIn("group: pulse-release-publish", workflow[publish:recheck])
        self.assertIn("cybex-pulse-qualified-predecessor.json", workflow)
        ...
        self.assertIn("case \"$CYBEX_PULSE_PUBLISHED_PREDECESSOR_UPDATE_CONTRACT\"", qualification_body)
        ...
        self.assertIn("cybex-pulse-ubuntu-update-qualification.json", workflow)
        self.assertIn("cybex.pulse.ubuntu-appliance-update-qualification.v1", workflow)
    ...
    def test_real_n_to_n_plus_one_harness_requires_activation_and_health(self) -> None:
        ...
        self.assertIn(
            'api POST "/v1/pulse/nodes/$server_device_id/qualification-updates"',
            harness,
        )
        ...
        self.assertIn("cybex.pulse.ubuntu-appliance-update-qualification.v1", harness)
...
class LocalPublishedPredecessorTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.served_prefix = "https://dev.example.test/pulse-dev-artifacts"
    ...
    def write_release(
        ...
        package_root.mkdir()
        repository.mkdir()
        updater = package_root / "usr/lib/cybex-pulse/cybex-pulse-appliance-update"
        packaged_release = package_root / "usr/share/cybex-pulse/appliance-release.json"
        ...
        if selective:
            updater.write_bytes(
                b"#!/usr/bin/env bash\n"
                b"# cybex.pulse.verified-appliance-update.v1\n"
                b"package_targets=(cybex-pulse cybex-pulse-bootstrap "
                b"cybex-pulse-appliance)\n"
        ...
        deb = repository / f"cybex-pulse-appliance_{version}-1_amd64.deb"
        subprocess.run(
            [
                "dpkg-deb",
                "--root-owner-group",
        ...
        for name, body in (
            ("Packages", b"Package: cybex-pulse-appliance\n"),
            ("Packages.gz", b"deterministic gzip fixture\n"),
            ("Release", b"Suite: resolute\n"),
        ):
        ...
        binary = directory / "cybex-pulse-x86_64-linux"
        template_name = (
            f"cybex-pulse-appliance-template-{version}-x86_64-linux.iso"
        )
        ...
        binary.write_bytes(f"pulse binary {version}\n".encode())
        ...
        manifest = {
            "schema": "cybex.pulse.release.v1",
            "version": version,
            "release_url": "https://dev.example.test/pulse",
            "notes_url": "https://dev.example.test/pulse",
        ...
        manifest_path = directory / "cybex-pulse-release.json"
        ...
        compatibility_path = directory / "cybex-pulse-release-compatibility.json"
    ...
    def write_stage_journal(
        ...
        journal = {
            "schema": "cybex.pulse.canonical-package-stage.v1",
            "owner": owner,
    ...
    def test_identifies_highest_exact_seven_file_local_release(self) -> None:
        ...
        self.assertEqual(
            identity["schema"],
            "cybex.pulse.local-published-appliance-predecessor.v1",
        )
    ...
    def test_local_release_rejects_extra_files_and_noncanonical_urls(self) -> None:
        ...
        selected.chmod(0o755)
        extra.unlink()
        manifest_path = selected / "cybex-pulse-release.json"
        manifest_path.chmod(0o644)
        manifest = json.loads(manifest_path.read_bytes())
        manifest["artifact"]["url"] = (
            "https://other.example.test/pulse-dev-artifacts/0.2.1-dev.12/"
            "cybex-pulse-x86_64-linux"
        )
        manifest_path.write_bytes(canonical(manifest))
        manifest_path.chmod(0o444)
        compatibility_path = selected / "cybex-pulse-release-compatibility.json"
        ...
        lines[-2] = f"{digest(manifest_path.read_bytes())} *cybex-pulse-release.json"
        lines[-1] = (
            f"{digest(compatibility_path.read_bytes())} "
            "*cybex-pulse-release-compatibility.json"
        )
    ...
    def test_production_workflow_is_byte_identical(self) -> None:
        ...
        self.assertEqual(
            digest(workflow),
            "ffbd5b198bd9c75d88135f4f78fe0a8bbdffa7d0579014f44f7db6075eac2e53",
        )
