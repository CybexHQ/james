...
INSTALLER_ISO_TEMPLATE_BUILD_SCHEMA = "cybex.pulse.installer-template-build.v1"
APPLIANCE_PACKAGE_SNAPSHOT_MAX_BYTES = 4 * 1024 * 1024 * 1024
MANAGE_SOURCE_METADATA_SCHEMA = "cybex.pulse.manage-source.v1"
...
PULSE_DEBIAN_PACKAGE_MAX_BYTES = 512 * 1024 * 1024
...
def _inspect_packaged_manage_source(
    ...
    expected_package_name = f"cybex-pulse_{version}-1_amd64.deb"
    with tempfile.TemporaryDirectory(prefix="cybex-pulse-manage-source-") as directory:
        directory_path = Path(directory)
        ...
        try:
            with tarfile.open(fileobj=decompressor.stdout, mode="r|") as archive:
                for member in archive:
                    ...
                    package_count += 1
                    if package_count != 1:
                        _fail("appliance package snapshot contains duplicate cybex-pulse packages")
                    if (
                        not member.isreg()
                        or member.islnk()
                        or member.issym()
                        ...
                        _fail("packaged cybex-pulse Debian metadata is not deterministic")
                    _copy_tar_member(
                        archive,
                        member,
                        package_path,
                        "packaged cybex-pulse Debian",
                        PULSE_DEBIAN_PACKAGE_MAX_BYTES,
                    )
        except ReleaseError:
            raise
        except (tarfile.TarError, OSError, EOFError):
        ...
        if package_count != 1:
            _fail("appliance package snapshot omits its exact cybex-pulse package")
        ...
        source_directory = "usr/share/cybex-pulse/manage-source"
        ...
        try:
                ...
                for member in package_archive:
                    ...
                    if name == source_directory:
                        if source_directory_seen:
                            _fail("cybex-pulse package contains a duplicate Manage source directory")
                    ...
                    if match is None or "/" in relative_name:
                        _fail("cybex-pulse package contains an unexpected Manage source entry")
                    revision, kind = match.groups()
                    if kind in entry_revisions:
                        _fail("cybex-pulse package contains duplicate Manage source entries")
            ...
            data_error = "cybex-pulse Debian data archive is malformed or truncated"
        finally:
        ...
        if (
            ...
            _fail("cybex-pulse package omits its exact Manage source archive contract")
