...
usage() {
  echo "usage: $0 --output-dir DIR --pulse-binary FILE --bootstrap-binary FILE --version SEMVER --ubuntu-snapshot-id ID --manage-source-dir DIR --manage-source-revision 40_HEX --expected-manage-origin HTTPS_ORIGIN --release-public-key BASE64 --provisioning-public-key BASE64 [--provisioning-public-key BASE64 ...] [--previous-package-snapshot FILE]" >&2
  exit 2
}
...
declare -a pulse_packages=()
while IFS= read -r -d '' package_file; do
  if [[ "$(dpkg-deb -f "$package_file" Package)" == cybex-pulse ]]; then
    pulse_packages+=("$package_file")
  fi
done < <(find "$offline_repository" -maxdepth 1 -type f -name '*.deb' -print0 | LC_ALL=C sort -z)
if [[ "${#pulse_packages[@]}" -ne 1 ]]; then
  echo "error: offline repository must contain exactly one cybex-pulse package" >&2
  exit 1
fi
packaged_pulse_root="$work_dir/packaged-pulse"
dpkg-deb --extract "${pulse_packages[0]}" "$packaged_pulse_root"
packaged_manage_source="$packaged_pulse_root/usr/share/cybex-pulse/manage-source"
