...
test "$(jq -er '.schema' "$predecessor_evidence")" = \
  cybex.pulse.ubuntu-appliance-qualification.v1
...
test "$(jq -er '.schema' "$candidate_manifest")" = cybex.pulse.release.v1
...
test "$(jq -er '.workstation_netboot.schema' "$candidate_manifest")" = \
  cybex.pulse.workstation-netboot.v1
...
test "${candidate_package_url##*/}" = \
  "cybex-pulse-appliance-packages-$candidate_release-x86_64-linux.tar.zst"
...
for package_name in cybex-pulse cybex-pulse-appliance cybex-pulse-bootstrap; do
...
fetch_runtime() {
  local destination="$1"
  api GET "/v1/pulse/nodes/$server_device_id/workstation-netboot" > "$destination"
...
fetch_node() {
  local destination="$1"
  if ! api GET "/v1/pulse/nodes/$server_device_id" > "$work_dir/node-detail.json"; then
    return 1
  fi
...
assert_healthy_node() {
  ...
  test -n "$(jq -er '.pulse_reported_at' "$node")"
}
...
api GET "/v1/pulse/nodes/$server_device_id/qualification-updates" \
...
before_reported_at="$(jq -er '.pulse_reported_at' "$before")"
final_reported_at="$(jq -er '.pulse_reported_at' "$final")"
...
jq -n \
  --arg schema 'cybex.pulse.ubuntu-appliance-update-qualification.v1' \
  ...
  '{schema:$schema,ok:true,
    ...
    candidate_binding:{isolated_appliance_candidate_admitted:true,
      ...
      package_transport:{kind:$transport_kind,
        ...
        signed_sha256_verified_by_terminal_pulse_receipt:true}},
    ...
    claims_not_made:["cache_poison_recovery_injected","manage_restart_injected",
      "pulse_service_restart_injected"],
