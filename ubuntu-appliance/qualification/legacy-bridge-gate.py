...
POLICY_SCHEMA = "cybex.pulse.legacy-update-bridge-policy.v1"
EVIDENCE_SCHEMA = "cybex.pulse.legacy-update-bridge-evidence.v1"
QUALIFICATION_SCHEMA = "cybex.pulse.ubuntu-appliance-qualification.v1"
INSTALLED_SCHEMA = "cybex.pulse.installed-appliance.v1"
PREDECESSOR_SCHEMA = "cybex.pulse.published-appliance-predecessor.v1"
LOCAL_PREDECESSOR_SCHEMA = (
    "cybex.pulse.local-published-appliance-predecessor.v1"
)
LOCAL_RELEASE_SET_SCHEMA = "cybex.pulse.local-immutable-release-set.v1"
LOCAL_RELEASE_INDEX_SCHEMA = "cybex.pulse.local-published-release-index.v1"
LOCAL_STAGE_LEDGER_SCHEMA = "cybex.pulse.canonical-package-stage.v1"
LEGACY_UPDATE_CONTRACT = "legacy_all_debs"
SELECTIVE_UPDATE_CONTRACT = "selective_roots_v2"
INSTALLED_RELEASE_PATH = Path("/usr/share/cybex-pulse/appliance-release.json")
...
UPDATER_PATH = Path("usr/lib/cybex-pulse/cybex-pulse-appliance-update")
...
MAX_PULSE_BINARY_BYTES = 512 * 1024 * 1024
...
def validate_snapshot_binding(
    ...
    if metadata["schema"] != "cybex.pulse.appliance-package-snapshot.v1":
...
def local_artifact_maximum(filename: str) -> int:
    if filename == "cybex-pulse-x86_64-linux":
        return MAX_PULSE_BINARY_BYTES
    if filename.startswith("cybex-pulse-appliance-template-"):
        return MAX_INSTALLER_TEMPLATE_BYTES
    if filename.startswith("cybex-pulse-appliance-packages-"):
...
def local_release_filenames(
    ...
    package_filename = (
        f"cybex-pulse-appliance-packages-{release_id}-x86_64-linux.tar.zst"
    )
    if entries == {package_filename}:
        return None
    template_filename = (
        f"cybex-pulse-appliance-template-{release_id}-x86_64-linux.iso"
    )
    fixed = {
...
def inspect_local_release_set(
    ...
    *,
    ...
    try:
        ...
        expected_modes = {
            name: 0o555 if name == "cybex-pulse-x86_64-linux" else 0o444
            for name in entries
        }
        sizes: dict[str, int] = {}
        for name in entries:
...
def local_published_release_index(
    ...
    for name in ordered_entries:
        ...
        if (
            ...
            package_filename = (
                f"cybex-pulse-appliance-packages-{name}-x86_64-linux.tar.zst"
            )
...
def verify_local_predecessor_descriptors(
    *,
    ...
    if compatibility.get("schema") != "cybex.pulse.release-compatibility.v1":
        fail("local published predecessor compatibility schema is invalid")
    if manifest.get("schema") != "cybex.pulse.release.v1":
    ...
    if compatibility.get("pulse_release_version") != release_id:
    ...
    binary_name = "cybex-pulse-x86_64-linux"
    ...
    require_local_asset_url(
        ...
        label="local published predecessor Pulse binary",
    )
    if binary.get("sha256") != binary_artifact["sha256"]:
        fail("local published predecessor Pulse binary identity is inconsistent")
    template_name = (
        f"cybex-pulse-appliance-template-{release_id}-x86_64-linux.iso"
    )
    ...
    try:
        ...
        connection.request(
            ...
            headers={
                ...
                "User-Agent": "Cybex-Pulse-Local-Predecessor/1",
            },
        )
...
def verify_published_predecessor_descriptors(
    *,
    ...
    if not isinstance(binary, dict):
        fail("published predecessor manifest has no Pulse binary descriptor")
    require_release_asset_url(
        binary.get("url"),
        tag_name=tag_name,
        filename="cybex-pulse-x86_64-linux",
        label="published predecessor Pulse binary",
        origin=origin,
    )
    appliance = manifest.get("appliance_release_v1")
...
def packaged_updater_identity(
    ...
    *,
    ...
    for package in packages:
        ...
        if package_name == "cybex-pulse-appliance":
    ...
    with tempfile.TemporaryDirectory(prefix="cybex-predecessor-package-") as directory:
        ...
        packaged_release = extraction / "usr/share/cybex-pulse/appliance-release.json"
    ...
    if packaged_release_value.get("schema") != "cybex.pulse.appliance-release.v1":
...
def validate_predecessor(
    ...
    if release.get("schema") != "cybex.pulse.appliance-release.v1":
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="Capture or verify a monotone legacy Pulse update bridge"
    )
