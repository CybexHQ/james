...
- the Cybex-branded GRUB boot screen used by the former USB installer, with a
  single **Boot Cybex Pulse Setup** action;
...
ubuntu-appliance/build-package-snapshot.sh \
  ...
  --manage-source-dir "$CYBEX_PULSE_MANAGE_SOURCE_DIR" \
  --manage-source-revision "$CYBEX_PULSE_MANAGE_SOURCE_REVISION" \
...
ubuntu-appliance/build-package-snapshot.sh \
  ...
  --release-public-key "$CYBEX_PULSE_UPDATE_TRUSTED_PUBLIC_KEY" \
  --provisioning-public-key "$CYBEX_PULSE_PROVISIONING_PUBLIC_KEY" \
  --previous-package-snapshot dist/cybex-pulse-appliance-packages-1.2.3-x86_64-linux.tar.zst
```
...
embeds it in `cybex-pulse` at
`/usr/share/cybex-pulse/manage-source/<revision>.tar`. Adjacent canonical JSON
...
build, Pulse securely reopens the revision-scoped Manage archive and metadata,
...
`tarball+file:///usr/share/cybex-pulse/manage-source/<revision>.tar` input; no
...
Names beginning with `cybex-pulse` are always discarded so each release uses
the newly built Pulse packages. No Debian or inner archive parser sees bytes
...
- `cybex-pulse-network-runtime.timer/service`: reconciles the advertised boot
...
administration or login screen. It shows only the administrator-assigned Pulse
...
unprivileged Pulse process or the dedicated `tftp` account write access. The
...
The Pulse appliance continues to require factory Secure Boot enabled. A
...
`cybex-pulse`, `cybex-pulse-bootstrap`, and `cybex-pulse-appliance`; only their
...
certificate, and then proves the greenfield delivery invariant. The new Pulse
...
so an official built-in that is incompatible with the current Pulse classifier
...
`/pulse-dev-artifacts` tree, first verify the complete candidate with
`tools/pulse-release.py verify`, then stage only the hash-bound package:

```bash
install -d -m 0700 /absolute/private/cybex-pulse-stage-state
ubuntu-appliance/qualification/stage-canonical-package.py stage \
  --manifest dist/cybex-pulse-release.json \
  --package-snapshot dist/cybex-pulse-appliance-packages-VERSION-x86_64-linux.tar.zst \
  --artifact-root /absolute/served/releases \
  --served-prefix https://manage.example/pulse-dev-artifacts \
  --state-dir /absolute/private/cybex-pulse-stage-state \
...
ubuntu-appliance/qualification/legacy-bridge-gate.py \
  ...
  --artifact-root /absolute/served/releases \
  --staging-state-dir /absolute/private/cybex-pulse-stage-state \
  --served-prefix https://manage.example/pulse-dev-artifacts \
  --trusted-public-key "$CYBEX_PULSE_UPDATE_TRUSTED_PUBLIC_KEY" \
  --release-verifier tools/pulse-release.py \
...
The separate
`cybex.pulse.local-published-appliance-predecessor.v1` result binds both the
